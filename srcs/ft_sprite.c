...
/*   Updated: 2021/05/11 16:20:15 by gpaeng           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sprite_dist(t_all *all)
{
	...
	{	//어디가 멀고 가까운지만 파악하기 위함으로 sqrt 안해도 됩니다.
		all->sprite[idx].distance = ((all->player.x - all->sprite[idx].x)
		* (all->player.x - all->sprite[idx].x)
		+ (all->player.y - all->sprite[idx].y)
		* (all->player.y - all->sprite[idx].y));
		idx++;
	}
	ft_rsort_sprite(all);
...
void	ft_sprite_conversion(t_all *all, int x)
{
	...
	(all->player.dir_y * all->sprite_ray.x
	- all->player.dir_x * all->sprite_ray.y);
	all->sprite_ray.transform_y = all->sprite_ray.inv_det *
	(-all->player.plane_y * all->sprite_ray.x
	+ all->player.plane_x * all->sprite_ray.y);
...
{
	//height
	all->sprite_ray.height = abs((int)(all->info.win_y
	/ all->sprite_ray.transform_y));
	all->sprite_ray.draw_start_y = -all->sprite_ray.height
	/ 2 + all->info.win_y / 2;
	all->sprite_ray.draw_end_y = all->sprite_ray.height / 2
	+ all->info.win_y / 2;
	...
	//width
	all->sprite_ray.width = abs((int)(all->info.win_y
	/ all->sprite_ray.transform_y));
	all->sprite_ray.draw_start_x = -all->sprite_ray.width / 2
	+ all->sprite_ray.screen_x;
	all->sprite_ray.draw_end_x = all->sprite_ray.width / 2
	+ all->sprite_ray.screen_x;
